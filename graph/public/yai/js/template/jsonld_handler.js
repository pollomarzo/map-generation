		...
		for (var template_dict of template_list) {
			...
			if (!template_dict['hide_descendants']) {
				...
				for (var p of template_dict['predicate_list']) {
					...
					if (template_dict['keys_to_hide_as_child'].includes(p) || template_dict['predicate_list'].length == 1) { // this predicate has been requested to be removed, by the template, save its children
...
function clean_tree(tree_dict_list) { // remove trees with empty text
	var new_tree_dict_list = [];
	for (var tree_dict of tree_dict_list) {
		if (tree_dict['text'] === null)
			continue
