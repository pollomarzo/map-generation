...
TEMPLATE_LIST = [
	...
	},
	{
		...
		'template_fn': (is_in_array, ancestor_predicate, value_list) => {
			...
			return get_formatted_tuple_template(is_in_array, value,
				x => `<strong>Question</strong>: ${x}`,
				x => `It has several <b>questions</b>`,
			)
		}
	},
	{
		...
		'template_fn': (is_in_array, ancestor_predicate, value_list) => {
			...
			if (super_classes) {
				if (isArray(super_classes)) {
					if (super_classes.filter(x => get_description(x, false).startsWith(PREFIX_MAP['wn'])).length > 1)
						super_classes = super_classes.filter(x => !get_description(x, false).startsWith(PREFIX_MAP['wn']));
		...
		'template_fn': (is_in_array, ancestor_predicate, value_list) => {
			...
			if (type_name == prefixed_string_to_uri('my:Unknown')) {
				return get_formatted_tuple_template(is_in_array, hasEntity,
					...
					x => `There are ${x.length} different <b>things</b> that have no class`,
				);
			}
			return get_formatted_tuple_template(is_in_array, hasEntity,
				x => `There is 1 <b>type</b> of ${type_name}: ${x}`,
				x => `There are ${x.length} different <b>types</b> of ${type_name}`,
			);
		}
	},
	{ // class instances
	...
	{
		'keys': ['my:triple', 'my:abstract', 'my:confidence', 'my:sentence', 'my:hasSource', 'my:source_id', 'my:syntactic_similarity', 'my:semantic_similarity', 'my:relevant_to', 'my:score'],
		'optional_keys': ['my:triple', 'my:abstract', 'my:hasSource', 'my:source_id', 'my:syntactic_similarity', 'my:semantic_similarity', 'my:relevant_to', 'my:score'],
		'hide_descendants': true,
		'template_fn': (is_in_array, ancestor_predicate, value_list) => {
			var [triple, abstract, pertinence, sentence, source, source_id, syntactic_similarity, semantic_similarity, relevant_to, score] = value_list;
			const p = prefixed_string_to_uri;
			...
			// console.log(`${sentence} is relevant to ${relevant_to}, score ${score}`);
			return get_table(head_list, [value_list]);
		}
	},
	{
